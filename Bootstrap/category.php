...
while ($call = mysqli_fetch_array($my_calls)){
	echo "<b>{$call['name']}</b> ({$call['phonenum']}) at {$call['time']}&emsp;Duration: {$call['duration']}<br><br>";
}
...
while ($text = mysqli_fetch_array($my_texts)){
	echo "<b>{$text['name']}</b> ({$text['phonenumber']}) at {$text['time']}<br>{$text['text_content']}<br><br>";
}
?>
<br><br>
