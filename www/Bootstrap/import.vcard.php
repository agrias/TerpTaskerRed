...
foreach($addresses as $address) {
  ...
  foreach($address as $type => $entries) {  	
	...
	$dest_addr['name'] = $dest_addr['firstname']." ".$dest_addr['lastname'];
    }
    ...
    if($type == "TEL") {
        ...
        // * Paste value in correct field.
        	  ...
        	  } elseif(self::checkType($entry, 'FAX'))  { $dest_addr['phone']    = $entry['VALUE'];  	  	
        	  } elseif(self::checkType($entry, 'WORK')) { $dest_addr['phone']   = $entry['VALUE'];
        	  } elseif(self::checkType($entry, 'CELL')) { $dest_addr['phone'] = $entry['VALUE'];  	  	
    	  } 
