...
var mytinytodo = window.mytinytodo = _mtt = {
	...
	init: function(options)
	{
		...
		$('#task_bar').keydown(function(event){
			if(event.keyCode == 27) {
				$(this).val('');
			}
		}).focusin(function(){
		...
		.keydown(function(event){
			...
			$('#search_placeholder').removeClass('placeholding');
			$('#toolbar').addClass('mtt-insearch');
			$(this).focus();
		}).focusout(function(){
			if('' == $(this).val()) $('#search_placeholder').addClass('placeholding');
		...
		$("#edittags").autocomplete('ajax.php?suggestTags', {scroll: false, multiple: true, selectFirst:false, max:8, extraParams:{list:function(){ var taskId = document.getElementById('taskedit_form').id.value; return [taskId].listId; }}});
		...
		$('#tasklist .taskminusbtn').live('click', function(){
			return false;
		});

		$('#tasklist .taskplusbtn').live('click', function(){
			var id = parseInt(getLiTaskId(this));
			if (id) estPlusBtn(this, id);
			return false;
		});

		$('#tasklist .taskminusbtn').live('click', function(){
			var id = parseInt(getLiTaskId(this));
			if (id) estMinusBtn(this, id);
			return false;
		});
	...
	loadLists: function(onInit)
	{
		...
		this.db.loadLists({id: userID}, function(res)
		{
			var ti = '';
			var openListId = 0;
			if(res && res.total)
...
//happens when you click the add new list (tab) button
function addList()
{
	//gets prompt from en.php, and the default text from class.default.php
	var r = prompt(_mtt.lang.get('addList'), _mtt.lang.get('addListDefault'));
	if(r == null) return;

	_mtt.db.request('addList', {name:r, id:userID}, function(json){
...
function addContext()
{
	...
	var selected = $('#contexts :selected').val();

	var r = prompt(_mtt.lang.get('addContext'), '');
	if(r == null) return;

	_mtt.db.request('addContext', {name:r, id:userID} );
	
	_mtt.db.request('getContexts', {id: userID}, function(json){
		prepareContexts(json.list, selected);
	});
};

function removeContext()
{
	var con_id = $("#contexts :selected").val();
	//prompt(s, '');
	_mtt.db.request('deleteContext', {id: con_id} );

	_mtt.db.request('getContexts', {id: userID}, function(json){
		prepareContexts(json.list, 0);
	});
}

//rename's the current list via popup dialog
...
//delete's current list and asks via popup if you're sure
function deleteCurList()
{
	...
	_mtt.db.request('deleteList', {list:curList.id}, function(json){
		if(!parseInt(json.total)) return;
		_mtt.loadLists({id:userID});
	})
};

//publishes or unpublishes current list
...
function prepareTaskStr(item, noteExp)
{
	...
	return '<li id="taskrow_'+id+'" class="' + (item.compl?'task-completed ':'') + item.dueClass + (item.note!=''?' task-has-note':'') +
		...
		prepareEstimate(item.estMins, item.estHours) + 
...
function prepareEstimate(mins, hrs)
{
	s = '<span class="task-estimate">' + hrs + ' hr ' + (mins == 0? '00' : mins) + '<div class="taskbtns"><a href="#" class="taskplusbtn"></a><a href="#" class="taskminusbtn"></a></div></span>';
	return s;
}
...
{
	if(form.task_bar.value == '') return false;
	_mtt.db.request('newTask', { list:curList.id, title: form.task_bar.value, tag:_mtt.filter.getTags(), id: userID }, function(json){
...
function changeTaskOrder(id)
{
	...
	// sortByDueDate
			...
			if(taskList[a].compl != taskList[b].compl) return taskList[a].compl-taskList[b].compl;
			if(taskList[a].dueInt != taskList[b].dueInt) return taskList[a].dueInt-taskList[b].dueInt;
			if(taskList[a].timeInt != taskList[b].timeInt) return taskList[a].timeInt - taskList[b].timeInt;
			if(taskList[a].prio != taskList[b].prio) return taskList[b].prio-taskList[a].prio;
			return taskList[a].ow-taskList[b].ow; 
		});
	// sortByDueDate (reverse)
			...
			if(taskList[a].compl != taskList[b].compl) return taskList[a].compl-taskList[b].compl;
			if(taskList[a].dueInt != taskList[b].dueInt) return taskList[b].dueInt-taskList[a].dueInt;
			if(taskList[a].timeInt != taskList[b].timeInt) return taskList[b].timeInt - taskList[a].timeInt;
			if(taskList[a].prio != taskList[b].prio) return taskList[a].prio-taskList[b].prio;
			return taskList[b].ow-taskList[a].ow; 
		});		
	// sortByDateCreated
...
function editTask(id)
{
	...
	//prompt(item.contexts, 'hi');
	_mtt.db.request('getContexts', {id: userID}, function(json){
		prepareContexts(json.list, item.contexts);
	});
	...
	form.estMins.value = item.estMins;
	form.duetime.value = item.duetime;
	form.estHours.value = item.estHours;
	form.contexts.value = item.contexts;
	form.emailReminder.value = item.ereminder;
	form.popupReminder.value = item.preminder;
...
function prepareContexts(list, selected)
{
	var s = '<select name="contexts" id ="contexts" class="select-class">';
	$.each(list, function(i,item){
		s = s + '<option value ="' + item.id + '"';
		if (item.id == selected)
		{
			s = s + ' selected="selected"';
		}
		s = s + '>' + item.title + '</option>';
	});
	s = s + '</select>';
	$('#contexts').replaceWith(s);
}
...
function clearEditForm()
{
	...
	form.duedate.value = '';
	form.prio.value = '0';
	form.estMins.value = '0';
	form.duetime.value = '00:00';
	form.contexts.value = '0';
	form.estHours.value = '0';
	form.id.value = '';

	//initialy sets the reminders to 'never'
	form.emailReminder.value = '-1';
	form.popupReminder.value = '-1';

	//then loads from db and replaces them by user defaults
	_mtt.db.request('getDefaults', {id: userID}, function(json){
		
		$('#emailReminder').val(json['emailReminder']);
		$('#popupReminder').val(json['popupReminder']);
	});
	toggleEditAllTags(0);
};
...
function showEditForm(isAdd)
{
	...
	if(isAdd)
	{
		...
		if($('#task_bar').val() != '')
		{
			_mtt.db.request('parseTaskStr', { list:curList.id, title:$('#task_bar').val(), tag:_mtt.filter.getTags() }, function(json){
				if(!json) return;
...
function saveTask(form)
{
	...
	_mtt.db.request('editTask', {id:form.id.value, title: form.task.value, note:form.note.value,
		prio:form.prio.value, tags:form.tags.value, duedate:form.duedate.value, estMins:form.estMins.value, estHours:form.estHours.value, contexts:form.contexts.value, 
		duetime:form.duetime.value, ereminder:form.emailReminder.value, preminder:form.popupReminder.value, userId:userID}, function(json){
			if(!parseInt(json.total)) return;
			var item = json.list[0];
...
function submitFullTask(form)
{
	...
	_mtt.db.request('fullNewTask', { list:curList.id, tag:_mtt.filter.getTags(), title: form.task.value, note:form.note.value,
			prio:form.prio.value, tags:form.tags.value, duedate:form.duedate.value, estMins:form.estMins.value, estHours:form.estHours.value, contexts:form.contexts.value,
			duetime:form.duetime.value, ereminder:form.emailReminder.value, preminder:form.popupReminder.value, id:userID}, function(json){
		if(!parseInt(json.total)) return;
		form.task.value = '';
		var item = json.list[0];
		taskList[item.id] = item;
...
function estPlusBtn(el, id)
{
	el.blur();
	var estMins = parseInt(taskList[id].estMins);
	var estHours = parseInt(taskList[id].estHours);
	//$('test');
	estMins = estMins + 15;
	if (estMins == 60)
	{
	...
	_mtt.db.request('setEst', {id:id, estMins: estMins, estHours: estHours});
	taskList[id].estMins = estMins;
	taskList[id].estHours = estHours;
	var $t = $('#taskrow_'+id);
	$t.find('.task-estimate').replaceWith(prepareEstimate(taskList[id].estMins, taskList[id].estHours));
	$t.effect("highlight", {color:_mtt.theme.editTaskFlashColor}, 'normal');
}

function estMinusBtn(el, id)
{
	el.blur();
	var estMins = parseInt(taskList[id].estMins);
	var estHours = parseInt(taskList[id].estHours);
